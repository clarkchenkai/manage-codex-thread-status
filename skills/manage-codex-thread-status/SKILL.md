...
- Existing threads may not reload later changes to global instructions or automation prompts. Validate new rules in a newly created thread or a new automation run.
...
3. Runtime: an active test thread changes from `✅` to `⏳`, `status` reads back `⏳`, and completion changes it to `✅`.
